...
def mk_df(date_info, aud_usd_info, eur_aud_info):
    ...
    df
    """
    date_info_ser = pd.Series(data=dict(date_info))
    aud_usd_ser = pd.Series(data=dict(aud_usd_info))
    eur_aud_ser = pd.Series(data=dict(eur_aud_info))

    df = pd.DataFrame({'AUD/USD': aud_usd_ser, 'EUR/AUD': eur_aud_ser, 'Dates': date_info_ser})

    df.index = df['Dates']
    cols = ['AUD/USD', 'EUR/AUD']
